...
async def update_handler(message: types.Message):
    ...
    state = dp.current_state()
    await state.finish()
    answer = f"Обновление аккуантов происходит здесь\n" \
             f"http://{DOMAIN}:6575/account\n" \
             f"http://{DOMAIN}:6576/account\n" \
             f"http://{DOMAIN}:6577/account\n" \
             f"http://{DOMAIN}:6578/account\n" \
             f"http://{DOMAIN}:6579/account\n"
